...
void sigintHandler(int signum) {
    // 접속자 수 1 감소
    chatinfo->count = chatinfo->count - 1;

    endwin(); // ncurses 종료
    printf("chatinfo->count: %d.\n", chatinfo->count);
    if (chatinfo->count == 0){
        printf("채팅이 종료되었습니다.");

        // 이전 채팅 기록을 지우고 공유 메모리 초기화
        int shmid = shmget(202155532, sizeof(CHAT_INFO), 0666);
        if (shmid >= 0) {
            // 공유 메모리 제거
...
int main(int argc, char **argv) {
    // 공유 메모리 초기화
//    memset(chatinfo, 0, sizeof(CHAT_INFO));
